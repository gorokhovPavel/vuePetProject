<template>
  ...
  >
    ...
    <a-checkbox
      v-if="typeTd === 3"
      v-show="isLineOrPolygon"
      class="mapObjIncludeReport"
      v-model="selectedItem"
      @change="setChangeTdOfMapObj(typeTd)"
    ></a-checkbox>
...
export default {
  ...
  created() {
    ...
    this.listOfColors = addExtension.getListColor();
    if (this.isLineOrPolygon) {
      this.selectedItem = this.row;
    }
    if (this.typeTd === 4) {
      this.selectedColor = addExtension.getValueFromNum(
        this.listOfColors,
        this.row
      );
    }
