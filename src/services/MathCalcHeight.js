...
export default class MathCalcHeight {
  ...
  async getHeightsLine(clickDraw, isReport, isMoreOneLayer = true) {
    ...
    return await this._lineHeights(
      clickDraw,
      lineWidth,
      lineStr,
      isReport,
  ...
  async getVolume(
    ...
    const allPointsObj = this._getAllPoint(
      arrCoordPoint,
      volumeAccurMeter,
      useThreeDCheck
    );
    const allVolumeData = await this._getHeightsAllLayersVolume(
      allPointsObj,
      isMoreOneLayer,
      useThreeDCheck
    );
    ...
    if (useThreeDCheck) {
      ...
      if (isMoreOneLayer && allVolumeData.length > 1) {
        ...
        polygonInfo = [
          ...
          `${
            !volumeStrNew
              ? ""
              : `${lang.getMessages("volumeMainLayer")} : ${volumeStrNew} <br/>`
          }
          ${
            !volumeStrOld
              ? ""
              : `${lang.getMessages("volumeAddLayer")}  : ${volumeStrOld} <br/>`
          }
          ${
            !volumeDiff
              ? ""
              : `${lang.getMessages("amountChangesVolumes")} : ${volumeDiff}`
          }`
        ];
      } else {
      ...
      finalRes = {
        ...
        area: squareForDb,
        totalVolume:
          volumeNew > 1000000000
            ? (volumeNew / 1000000000).toFixed(0)
            : volumeNew.toFixed(0),
  ...
  getTotalDistanceStr(objDraw, isTypeStr) {
    ...
    if (objDraw.geometry.type === "LineString") {
      ...
      if (isTypeStr) {
        outStr = `${lang.getMessages("width")} : ${this._setLineToStr(
          lineDistance,
          2
        )}`;
      } else {
      ...
      let perimetr = turf.lineDistance(
        turf.lineString(objDraw.geometry.coordinates[0])
      );
      let strPerimetr = `${lang.getMessages("perimetr")} : ${this._setLineToStr(
        perimetr,
        2
      )} ;`;
      ...
      let strArea =
        area != 0
          ? `${lang.getMessages("square")} : ${this._setLineToStr(area, 3)} ^2`
          : "";

      if (isTypeStr) outStr = strPerimetr + strArea;
      else outStr = area;
    } else {
      outStr = "-1";
    }
  ...
  getStartCoord(thisDrawItem) {
    const resCoord =
      thisDrawItem.geometry.coordinates[0][0] != undefined &&
      thisDrawItem.geometry.coordinates[0][0].length == 2
        ? thisDrawItem.geometry.coordinates[0][0]
        : thisDrawItem.geometry.coordinates[0] != undefined &&
          thisDrawItem.geometry.coordinates[0].length == 2
  ...
  _getAllPoint(inArrCoordPoint, inVolumeAccurMeter, inUseThreeDCheck) {
    ...
    let widthOfSquare = turf.lineDistance(
      turf.lineString([extremData[0], extremData[3]])
    );
    let isSquarePolygon = inUseThreeDCheck;
    startCoordInLineArr = this._getArrLinePoints(
      [extremData[0], extremData[1]],
      inVolumeAccurMeter,
      widthOfSquare,
      isSquarePolygon
    );
    finCoordInLineArr = this._getArrLinePoints(
      [extremData[3], extremData[2]],
      inVolumeAccurMeter,
      widthOfSquare,
      isSquarePolygon
    );
    ...
    for (let i = 0; i < startCoordInLineArr.length; i++) {
      ...
      let nextTurfLine = turf.lineString([
        startCoordInLineArr[i],
        finCoordInLineArr[i]
      ]);
      let curCoordStEnd = [];
      ...
      if (inUseThreeDCheck) {
        ...
        arrInterPoint = this._getArrLinePointsAll(
          curCoordStEnd,
          startCoordInLineArr.length - 1,
          i,
          isSquarePolygon
        );
      } else {
        ...
        if (curCoordStEnd.length) {
          ...
          let curCoordStEndLength =
            this._getArrLinePoints(
              curCoordStEnd,
              inVolumeAccurMeter,
              widthOfSquare,
              isSquarePolygon
    ...
    let allPointsData = {
      ...
      pointsPerLine: inUseThreeDCheck
        ? startCoordInLineArr.length
        : arrPointsFor3d.length
    };
  ...
  _getArrLineStrForTurfIntersect(inArrCoordOfPolygon) {
    ...
    inArrCoordOfPolygonU.push([
      inArrCoordOfPolygon[0][0],
      inArrCoordOfPolygon[0][1]
    ]);
    let turfLsArr = [];

    for (let i = 0; i < inArrCoordOfPolygonU.length; i++) {
      if (inArrCoordOfPolygonU[i + 1]) {
        let turfLs = turf.lineString([
          inArrCoordOfPolygonU[i],
          inArrCoordOfPolygonU[i + 1]
        ]);
  ...
  async _lineHeights(
    ...
    if (widthLine === "-1") {
      ...
      this._getAllSelectedLayers(false, true).forEach((x, index) => {
        if (x) currIndex = index;
      });
      const currHeights = await this._getHeights(
        upArrCoordPoint,
        currIndex,
        "line",
        0
      ...
      const arrPoint = this._getArrLinePoints(
        arrCoordPoint,
        null,
        widthLine,
        false
      );
      const arrHeight = await this._getHeightsAllLayersLine(
        arrPoint,
        isMoreOneLayer
      );
  ...
  _getAllSelectedLayers(inIsMoreOneLayer, inUseThreeDCheck) {
    let startSelectedDataCheckArr = !inUseThreeDCheck
      ...
      : this.selectedDataCheckArr.map(
          (item, index) => index === this.indMain || index === this.indAdd
        );
  ...
  async _getHeightsAllLayersVolume(
    inArrPoint,
    inIsMoreOneLayer,
    inUseThreeDCheck
  ) {
    let arrAllVolume = [];
    const checkLayersArr = this._getAllSelectedLayers(
      inIsMoreOneLayer,
      inUseThreeDCheck
    );
    ...
    for (let i = 0; i < checkLayersArr.length; i++) {
      if (checkLayersArr[i]) {
        const arrHeightNew = await this._getHeights(
          inArrPoint.arrGlobalInterPoint,
          i,
          "volume",
          inArrPoint.pointsPerLine
        );
        //Парсинг предыдущих двумерных массивов в одномерные для 3д
        const arrHeightsFor3dNew = this._getPartHeights(
          inArrPoint.partArrPoint,
          arrHeightNew
        );
        //Тянем объем полигона старого и нового слоев, а также формируем строки с инфой и разницу объемов
        const volumeNew = this._getNumericalIntVolume(
          this.dxVal,
          arrHeightsFor3dNew
        );
        const volumeStrNew = this._setLineToStr(volumeNew, 1);
        const nameDataVolume = this.mapModel.layers.find(
          (elem, index) => index === i
        ).createDtStr;
  ...
  async _getHeightsAllLayersLine(inArrPoint, inIsMoreOneLayer) {
    ...
    for (let i = 0; i < checkLayersArr.length; i++) {
      if (checkLayersArr[i]) {
        const arrHeight = await this._getHeights(
          inArrPoint,
          i,
          "line",
          inArrPoint.length
        ...
        if (arrHeight) {
          const nameHeight = this.mapModel.layers.find(
            (elem, index) => index === i
          ).createDtStr;
          const dataHeight = {
            array: arrHeight,
  ...
  _getMathDataFromArr(inArr) {
    if (!inArr || inArr.length < 2) return;
  ...
  _getArrLinePoints(arrInpCoord, inAccurMeter, distanseLine, squarePolygon) {
    ...
    return this._getArrLinePointsAll(
      arrInpCoord,
      stepCount,
      null,
      squarePolygon
  ...
  async _getHeights(arrInpPoint, indexForRequest, type, pointsPerLine) {
    ...
    if (failIndex || !arrInpPoint || !type) return;

    let zeroMark = 0;
    let skyHeightsIdDb = this.mapModel.layers[indexForRequest].id;
    let skyHeightsName = this.mapModel.heightsMapNameConfigList[
      indexForRequest
    ];
    ...
    let subRequests = [];
    let data2 = addExtension.getChunksArray(
      arrInpPoint.slice(0),
      pointsPerRequest
    );
    ...
    for (let k = 0; k < data2.length; k++) {
      ...
      if (data2[k].length > 1000) {
        subDataArr = addExtension.getChunksArray(data2[k].slice(0), 1000);
      } else {
        subDataArr = [data2[k]];
      }
      ...
      for (let s = 0; s < subDataArr.length; s++) {
        ...
        rangePoints.push(
          geolib.computeDestinationPoint(
            { latitude: latMax, longitude: longMax },
            addMeters,
            0
          )
        );
        ...
        rangePoints.push(
          geolib.computeDestinationPoint(
            { latitude: latMax, longitude: longMin },
            addMeters,
            0
          )
        );
        ...
        rangePoints.push(
          geolib.computeDestinationPoint(
            { latitude: latMin, longitude: longMin },
            addMeters,
            0
          )
        );
        ...
        rangePoints.push(
          geolib.computeDestinationPoint(
            { latitude: latMin, longitude: longMax },
            addMeters,
            0
          )
        );
    ...
    let dataPerRequest = addExtension.getChunksArray(
      subRequests.slice(0),
      itemsPerRequest
    );
    let outHeightsArr2 = [];
    ...
    for (let m = 0; m < dataPerRequest.length; m++) {
      await api
        ...
        .then(response => {
          let localRes = addExtension.setParseJsonToData(
            response.data,
            zeroMark
          );
  ...
  _getPerimetr(obj) {
    const perimetr = turf.lineDistance(
      turf.lineString(obj.geometry.coordinates[0])
    );
